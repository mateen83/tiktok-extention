...
class DownloadQueue {
  ...
  async resolveVideoUrl(pageUrl) {
    const apis = [
      {
        ...
        fetch: async (url) => {
          ...
          if (json.code !== 0 || !json.data) throw new Error(json.msg || 'API error');
      ...
      },
      {
        name: 'lovetik.com',
        fetch: async (url) => {
          const API_BASE = 'https://lovetik.com/api/ajax/search';
          const response = await fetch(API_BASE, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          ...
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          const json = await response.json();
          if (json.status !== 'ok' || !json.links) throw new Error('API error');
          
          // Fallback options ft:1 means no watermark/HD video
          const validLinks = json.links.filter(l => l.ft === 1 && l.a);
          if (validLinks.length === 0) throw new Error('No no-watermark video URL');
          return validLinks[0].a;
        }
      },
      {
        name: 'api.tikmate.app',
        fetch: async (url) => {
          const API_BASE = 'https://api.tikmate.app/api/lookup';
          const response = await fetch(API_BASE, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          ...
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          const json = await response.json();
          if (!json.success || !json.token || !json.id) throw new Error('API error or no video');
          return `https://tikmate.app/download/${json.token}/${json.id}.mp4?hd=1`;
        }
      },
      {
        name: 'tikwm.com GET fallback',
        fetch: async (url) => {
          const fallbackUrl = `https://tikwm.com/api/?url=${encodeURIComponent(url)}&hd=1`;
    ...
    for (const api of apis) {
      try {
        console.log(`[TTDL] Trying API: ${api.name}`);
        const videoUrl = await api.fetch(pageUrl);
        if (videoUrl) {
          console.log(`[TTDL] Resolved HD video URL using ${api.name}`);
          return videoUrl;
        }
      } catch (err) {
        console.error(`[TTDL] API (${api.name}) failed:`, err.message);
      }
    }
